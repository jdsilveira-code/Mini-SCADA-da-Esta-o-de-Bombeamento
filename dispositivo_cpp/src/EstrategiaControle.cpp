...
// Implementação de EstrategiaManual
EstrategiaManual::EstrategiaManual()
    : EstrategiaControle(0.0f), AcaoBombaAtual(AcaoBomba::AUTOMATICO), PotenciaBomba(0.0f), AcaoVaretas(AcaoVaretas::AUTOMATICO) {}
...
// Implementação de EstrategiaAutomatica
...
void EstrategiaAutomatica::aplicar(Sensor* sensor, Atuador* atuador) {
    ...
    BombaAgua* bomba = dynamic_cast<BombaAgua*>(atuador);
    if (bomba) {
        // Controle automático da bomba delega para o ControleBombaAutomatico
    ...
    Varetas* varetas = dynamic_cast<Varetas*>(atuador);
    if (varetas) {
        // Controle automático das varetas (queima) usa ControleQueima
